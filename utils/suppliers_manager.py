...
class SuppliersManager:
    ...
    def search_by_code(self, code: str) -> Optional[Dict]:
        ...
        if self.data is None or len(self.data) == 0:
            logger.warning("No data loaded in suppliers_manager")
            return None
        
        try:
            # ค้นหาในคอลัมน์ที่ 2 (Code)
            code_col = self.data.columns[2]
            logger.debug(f"Searching for code '{code}' in column '{code_col}'")
            logger.debug(f"Available codes: {self.data[code_col].head().tolist()}")
            result = self.data[self.data[code_col].astype(str).str.strip() == code.strip()]
            
            if len(result) > 0:
                logger.debug(f"Found match for code '{code}'")
                return result.iloc[0].to_dict()
            logger.debug(f"No match found for code '{code}'")
            return None
        except Exception as e:
